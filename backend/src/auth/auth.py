...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'bad_request',
            'description': 'Account does not have any permissions set.'
        }, 400)

    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'unauthorized',
            'description': 'Account missing permissions for this action.'
        }, 401)
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'invalid_claims',
                'description': 'Please check the audidence and issuer.'
            })
